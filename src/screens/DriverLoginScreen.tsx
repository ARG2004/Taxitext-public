...
export default function DriverLoginScreen({ navigation }: any) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (uid) {
        ...
        if (doc.exists() && doc.data()?.rol === 'conductor') {
          navigation.replace('DriverHome');
        } else {
          await auth().signOut();
          Alert.alert('Acceso denegado', 'Esta cuenta no es de conductor. Usa el login de pasajero.');
        }
      }
    } catch (error: any) {
  ...
  const handleRegister = async () => {
    if (!nombre.trim() || !email.trim() || !password.trim() || !telefono.trim() || !placa.trim() || !numUnidad.trim()) {
      ...
      <Animated.View style={[styles.header, { transform: [{ translateY: headerMove }] }]}>
        ...
        <View style={styles.headerContent}>
          <Animated.View style={[styles.avatarCircle, { transform: [{ scale: avatarScale }] }]}>
             {isLogin ? (
                <LogIn size={38} color={Colors.secondary} strokeWidth={2.5} />
             ) : (
                <Car size={38} color={Colors.secondary} strokeWidth={2.5} />
             )}
          </Animated.View>
          <Animated.Text style={[styles.headerTitle, { opacity: fadeAnim }]}>
            {isLogin ? 'Acceso Conductor' : 'Registro Conductor'}
          </Animated.Text>
          <Animated.Text style={[styles.headerSub, { opacity: fadeAnim }]}>
            {isLogin ? 'Inicia sesión para recibir viajes' : 'Únete al equipo Taxitex'}
          </Animated.Text>
        </View>
      </Animated.View>
        ...
        >
          ...
          {!isLogin && (
            <View>
              ...
              <View style={styles.inputWrap}>
                <Hash size={20} color={Colors.textSecondary} strokeWidth={2} />
                <TextInput
                  style={styles.input}
              ...
              <View style={styles.inputWrap}>
                <Car size={20} color={Colors.textSecondary} strokeWidth={2} />
                <TextInput
                  style={styles.input}
                  placeholder="Ej: 042"
                  placeholderTextColor={Colors.textMuted}
                  value={numUnidad}
                  onChangeText={setNumUnidad}
          ...
          >
            {loading ? (
              ...
              <View style={styles.btnContent}>
                ...
                <ArrowRight size={20} color={Colors.textOnSecondary} strokeWidth={3} />
              </View>
            )}
          </TouchableOpacity>
